...
export default upstashWrappedResponseHandler(async () => {
  ...
  const seriesData = await getSeriesData('ELMS')
  ...
  const data = await Promise.all(
    filteredResults.map(({ season, event, results }) => {
      return Promise.all(
        results.map(result =>
          $fetch<string>(`http://elms.alkamelsystems.com/${result}`)
            .then(data => ({
  ...
  return { updatedELMS: updated.length }
})
...
async function getSeasonsWithEvents() {
  ...
  const seasonsData = Promise.all(
    seasons.map(season =>
      $fetch<string>(`http://elms.alkamelsystems.com/?season=${season}`)
        .then((data) => {
          return {
            data,
            season,
