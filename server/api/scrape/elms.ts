...
export default defineEventHandler(async () => {
  ...
  let seriesData = await db.query.series.findFirst({
    where: (series, { like }) => like(series.name, 'ELMS'),
  ...
  const data = await Promise.all(
    filteredResults.map(({ season, event, results }) => {
      return Promise.all(
        results.map(result =>
          $fetch<string>(`http://elms.alkamelsystems.com/${result}`).then(
            data => ({
              url: `http://elms.alkamelsystems.com/${result}`,
  ...
  return { updatedELMS: data.length }
})

async function getSeasonsWithEvents() {
  const html = await $fetch<string>('http://elms.alkamelsystems.com/')
  ...
  const seasonsData = Promise.all(
    seasons.map(season =>
      $fetch<string>(`http://elms.alkamelsystems.com/?season=${season}`).then(
        data => {
          return {
            data,
            season,
  ...
  const data = await Promise.all(
    seasonsWithEvents.map(({ season, events }) => {
      return Promise.all(
        events.map(event =>
          $fetch<string>(
            `http://elms.alkamelsystems.com/?season=${season}&evvent=${event}`
...
function parseEventResults(
  ...
  const results = []
  for (const t of td) {
    if (t.text.includes('EUROPEAN LE MANS SERIES')) {
