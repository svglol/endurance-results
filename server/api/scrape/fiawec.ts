...
export default defineEventHandler(async () => {
  ...
  const data = await Promise.all(
    filteredResults.map(({ season, event, results }) => {
      return Promise.all(
        results.map(result =>
          $fetch<string>(`http://fiawec.alkamelsystems.com/${result}`)
            .then(data => ({
  ...
  return {
    updatedFIAWEC: data.flatMap(d => d.results).filter(r => r.data !== '')
      .length,
  }
})
...
async function getSeasonsWithEvents() {
  ...
  const seasonsData = Promise.all(
    seasons.map(season =>
      $fetch<string>(`http://fiawec.alkamelsystems.com/?season=${season}`)
        .then(data => {
          return {
            data,
            season,
